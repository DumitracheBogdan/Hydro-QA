...
const OUTPUT_PATH = path.join(QA_ARTIFACTS, 'hydrocert_no_login_60_findings_report.html');
...
function getLatestNoLogin60RunDir() {
  if (!fs.existsSync(EVIDENCE_ROOT)) return null;
  const dirs = fs
    .readdirSync(EVIDENCE_ROOT, { withFileTypes: true })
    .filter((d) => d.isDirectory() && d.name.startsWith('no-login-60-findings-'))
...
function annotatedScreenshotFor(absPath) {
  const ext = path.extname(absPath);
  if (!/\.(png|jpg|jpeg|webp)$/i.test(ext)) return absPath;
  const dir = path.dirname(absPath);
  const base = path.basename(absPath);
  const candidates = [
    path.join(dir, `ann-${base}`),
    path.join(dir, 'ann-05-row-evidence.png'),
  ];
...
function buildEvidenceHtml(finding) {
  ...
  items.forEach((raw, idx) => {
    ...
    if (['.png', '.jpg', '.jpeg', '.webp'].includes(ext)) {
      const evPath = annotatedScreenshotFor(abs);
      const rel = toRel(evPath);
      chunks.push(`
      <div class="screenshot-container">
        <h5>Screenshot Evidence ${idx + 1}</h5>
        <a href="${escapeHtml(rel)}" target="_blank"><img src="${escapeHtml(rel)}" alt="${escapeHtml(path.basename(evPath))}"></a>
      </div>`);
      return;
    }
...
function run() {
  const runDir = getLatestNoLogin60RunDir();
  if (!runDir) {
    throw new Error('No no-login-60-findings run directory found.');
  }
  ...
  if (findings.length !== 60) {
    throw new Error(`Expected exactly 60 findings, got ${findings.length}`);
  }
...
<head>
  ...
  <title>HydroCert QA Test Report - 60 Findings (No Login)</title>
  <style>
...
<body>
  <div class="container">
    <header>
      <h1>HydroCert QA Test Report - 60 Findings (No Login)</h1>
