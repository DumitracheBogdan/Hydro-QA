...
test("patch sends PATCH with bearer token and json body", async () => {
  const calls = [];
  const fakeFetch = async (url, opts) => {
    calls.push({ url, opts });
    if (url.endsWith("/auth/login")) return { ok: true, status: 200, json: async () => ({ tokens: { accessToken: "TKN" }, user: { id: "U1" } }) };
    return { ok: true, status: 200, json: async () => ({ id: "V1", waterSystemDescription: "x" }) };
  };
  const c = makeClient("https://api.dev.example", fakeFetch);
  await c.login("a@b.c", "pw");
  await c.patch("/visits/V1", { waterSystemDescription: "x" });
  assert.equal(calls[1].opts.method, "PATCH");
  assert.equal(calls[1].opts.headers.Authorization, "Bearer TKN");
  assert.equal(JSON.parse(calls[1].opts.body).waterSystemDescription, "x");
});
