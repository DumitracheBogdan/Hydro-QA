...
const OUTPUT_PATH = path.join(QA_ARTIFACTS, 'hydrocert_no_login_50_more_findings_v2_report.html');
...
function latestRunDir() {
  ...
  const dirs = fs
    ...
    .filter((d) => d.isDirectory() && d.name.startsWith('no-login-50-more-findings-v2-'))
...
function run() {
  ...
  if (!runDir) throw new Error('No run dir found for no-login-50-more-findings-v2');
  const summaryPath = path.join(runDir, 'summary.json');
  if (!fs.existsSync(summaryPath)) throw new Error(`summary.json missing in ${runDir}`);
  const summary = JSON.parse(fs.readFileSync(summaryPath, 'utf-8'));
  const findings = Array.isArray(summary.findings) ? summary.findings : [];
  if (findings.length !== 50) throw new Error(`Expected 50 findings, got ${findings.length}`);
...
<head>
  ...
  <title>HydroCert QA Test Report - Additional 50 Findings</title>
  <style>
...
<body>
  <div class="container">
    <header>
      <h1>HydroCert QA Test Report - Additional 50 Findings</h1>
