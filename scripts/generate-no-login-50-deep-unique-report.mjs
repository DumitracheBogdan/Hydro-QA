...
const OUTPUT_PATH = path.join(QA_ARTIFACTS, 'hydrocert_no_login_50_deep_unique_findings_report.html');
...
function latestRunDir() {
  ...
  const dirs = fs
    ...
    .filter((d) => d.isDirectory() && d.name.startsWith('no-login-50-deep-unique-'))
...
function run() {
  ...
  if (!runDir) throw new Error('No run dir found for no-login-50-deep-unique');
  const summaryPath = path.join(runDir, 'summary.json');
  if (!fs.existsSync(summaryPath)) throw new Error(`summary.json missing in ${runDir}`);
  const summary = JSON.parse(fs.readFileSync(summaryPath, 'utf-8'));
  const findings = Array.isArray(summary.findings) ? summary.findings : [];
  if (findings.length < 50) throw new Error(`Expected at least 50 findings, got ${findings.length}`);
...
<head>
  ...
  <title>HydroCert QA Test Report - 50 Deep Unique Findings</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .bug-card { border: 1px solid #e0e0e0; border-radius: 6px; padding: 15px; margin: 12px 0; border-left: 4px solid #dc3545; background: #fff; }
    ...
    .bug-card.low, .bug-card.unknown { border-left-color: #17a2b8; background: #f8fcff; }
    ...
    pre { margin-top: 8px; background: #111827; color: #d1d5db; padding: 10px; border-radius: 8px; white-space: pre-wrap; word-break: break-word; }
...
<body>
  <div class="container">
    <header>
      <h1>HydroCert QA Test Report - 50 Deep Unique Findings</h1>
    ...
    <section class="section">
      ...
      <div class="summary-grid">
        <div class="summary-card critical"><div class="number">${grouped.critical.length}</div><div class="label">Critical Bugs</div></div>
        <div class="summary-card fail"><div class="number">${grouped.high.length}</div><div class="label">High Bugs</div></div>
        <div class="summary-card warning"><div class="number">${grouped.medium.length}</div><div class="label">Medium Bugs</div></div>
        <div class="summary-card info"><div class="number">${grouped.low.length + grouped.unknown.length}</div><div class="label">Low/Other Issues</div></div>
        <div class="summary-card info"><div class="number">${findings.length}</div><div class="label">Total Issues</div></div>
      </div>
    </section>
