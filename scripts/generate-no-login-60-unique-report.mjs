...
const OUTPUT_PATH = path.join(QA_ARTIFACTS, 'hydrocert_no_login_60_unique_findings_report.html');
...
function latestRunDir() {
  ...
  const dirs = fs
    ...
    .filter((d) => d.isDirectory() && d.name.startsWith('no-login-60-unique-findings-'))
...
function run() {
  ...
  if (!runDir) throw new Error('No run dir found for no-login-60-unique-findings');
  const summaryPath = path.join(runDir, 'summary.json');
  if (!fs.existsSync(summaryPath)) throw new Error(`summary.json missing in ${runDir}`);
  const summary = JSON.parse(fs.readFileSync(summaryPath, 'utf-8'));
  const findings = Array.isArray(summary.findings) ? summary.findings : [];
  if (findings.length < 60) throw new Error(`Expected at least 60 findings, got ${findings.length}`);
...
<head>
  ...
  <title>HydroCert QA Test Report - 60 Unique Findings</title>
  <style>
...
<body>
  <div class="container">
    <header>
      <h1>HydroCert QA Test Report - 60 Unique Findings</h1>
