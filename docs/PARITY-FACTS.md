...
- **Mobile login user:** dedicated engineer `parity.bot@hydrocert.com` (password in GH secret `HYDROCERT_PARITY_MOBILE_PASSWORD` / Obsidian `credentials.md` — NOT stored in this public repo). The password is deliberately alphanumeric (no special chars) — Maestro `inputText` mistypes chars like `!`, which caused false "Invalid Credentials".
...
| 3c-risk | mobile→web | p04 | Risk Assessment "- Comments" field (1 automated on CI; see note) |
...
- **Risk Assessment "- Comments" fields** — p04 is generated by `scripts/parity/gen-p04.mjs` from `RISK_COMMENT_FIELDS_AUTOMATED` in `setup-data.mjs` (a slice of the full 18-field `RISK_COMMENT_FIELDS` backend truth). Both the flow and `buildExpected().riskAssessment` consume the same constant so they never drift. RA is the LAST card → after the final field, scroll DOWN to "Save" directly (collapsing by scrolling UP to the header is unreliable from the bottom of the long card).
  - **CI automates 1 field, not 18 (verified 2026-05-27).** The full 18-field flow types + saves correctly on a local emulator (API 35) but fails on the CI emulator (API 30): for the 2nd+ field, `scrollUntilVisible(label)` lands the label at the bottom edge with its input **below the fold**, so `tapOn { below: label }` either errors (input off-screen) or, with `centerElement: true`, taps the wrong node and the value is never persisted (3c reads back all-null). `waitForAnimationToEnd` settles did not help (it's geometry, not a focus race). The short p05 visit-text form (3 fields, inputs on-screen) uses the same `tapOn below` pattern and passes on CI — confirming it's the long-form scroll geometry, not the pattern. To re-attempt 18 on CI, widen `RISK_COMMENT_FIELDS_AUTOMATED` and make each field scroll to the *next* label as the anchor (so the current field's input stays on-screen).
